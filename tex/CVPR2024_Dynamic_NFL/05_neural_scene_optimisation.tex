...
% \paragraph{Ray drop labels for dynamic neural field} 
% We select ray drop labels for training dynamic neural fields as follows. Given a ray intersected with a vehicle's bounding box $v$, if $v$ matches the ray label $\mathcal{I}$, it is treated as a \textbf{non-dropped} sample ($\hat{p_d} = 0$). Conversely, if the ray either \textbf{1.)} is marked as dropped in the dataset or \textbf{2.)} extends beyond the vehicle's bounding box range, it is considered a \textbf{dropped} sample ($\hat{p_d} = 1$). Utilizing these labels, we train the ray drop prediction for each dynamic neural field using the same loss as \ref{eq:raydrop_loss}.



% \paragraph{Implementation details}\label{sec:trainingdetails}

% In training time We choose $w_{r}=3, w_{in}=50, w_{*}=0.15, w_{s}=1, w_{\text{eik}}=0.3 $, batch size as 4096 and trained for 60000 iterations on a single RTX3090 GPU, the starting learning rate is 0.005. 