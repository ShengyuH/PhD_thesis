...
% We extracted 50 consecutive frames from each scene, covering frame indices ranging from 50 to 100. Within each scene, our criteria for classifying a vehicle as dynamic involved assessing if the vehicle showcased a speed greater than 1 $m/s$ in any one of the 50 frames. Utilizing the bounding box annotations provided for these vehicles, we marked the points residing within these bounding boxes as the designated vehicle points for further analysis and annotation.
...
% such that we acknowledge which ray segment $[\zeta_{\text{near}}, \zeta_{\text{far}}]$ lies within the bounding box. 
% We then uniformly sample 512 points along the ray and query the SDF values based on the intersection results. If the point lies within any intersected ray segment$[\zeta_{near}, \zeta_{far}]$ of a dynamic vehicle $v$, we query the SDF $s$, intensity $e$ and ray drop probability $p_d$ in the corresponding dynamic neural field $F^v$. If the point does not lie in any vehicle bounding box, we sample the SDF $s$, intensity $e$ and ray drop probability $p_d$ in the static neural field $F_{static}$.

% and extracted the SDF values. If a sampled point fell within any intersected ray segment $[\zeta_{\text{near}}, \zeta_{\text{far}}]$ of a dynamic vehicle $v$, we extracted the corresponding SDF $s$, intensity $e$, and ray drop probability $p_d$ from the associated dynamic neural field $\mathbf{F}^{\text{v}}$. Conversely, if the point didn't reside within any vehicle bounding box, we sampled the SDF $s$, intensity $e$, and ray drop probability $p_d$ from the static neural field $\mathbf{F}_{\text{static}}$.